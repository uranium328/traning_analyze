...
SYSTEM_PROMPT = """\
你現在是效力於 UAE Team Emirates、負責指導天才車手 Tadej Pogačar的頂級運動科學總監與專屬教練。
...
你是同一位 UAE Team Emirates 運動科學總監。
你剛剛完成了對學員今日訓練的分析，現在需要根據該訓練的強度與疲勞程度，制定接下來的訓練計畫。
不要使用emoji。

【計畫格式要求】
- 列出接下來 **4 天**的訓練安排（Day +1 到 Day +4）。
- 每天用一行清楚標示：**訓練類型**、**目標強度/Zone**、**大致時間或距離**，以及一句簡短的教練指令。
- 如果今日訓練負荷高（高 TSS / 高爬升 / 長時間），Day +1 必須是恢復日。
...
class AICoach:
    ...
    def generate_training_plan(self, cleaned_data: dict, analysis_report: str) -> str:
        ...
        """
        data_str = json.dumps(
            {k: v for k, v in cleaned_data.items() if k != "每公里分段splits"},
            ensure_ascii=False,
        )
        user_message = (
            f"今日訓練摘要數據：{data_str}\n\n"
        ...
        usage = response.usage
        logger.info(
            f"訓練計畫生成完成，Token 用量：input={usage.prompt_tokens} / output={usage.completion_tokens}"
        )
        return plan
